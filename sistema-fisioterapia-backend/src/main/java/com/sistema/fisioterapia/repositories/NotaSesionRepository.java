...
public interface NotaSesionRepository extends JpaRepository<NotaSesion,Long> {
    ...
    @Query(value="SELECT c.* FROM nota_sesion c, sesion d WHERE c.sesion_id_id = d.id and d.paciente_id_usuario_id=:paciente_id and d.lesion_id_lesion_id=:lesion_id", nativeQuery=true)
