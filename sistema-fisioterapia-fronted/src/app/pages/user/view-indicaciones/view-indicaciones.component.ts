...
export class ViewIndicacionesComponent implements OnInit {
  ...
  constructor(private userService:UserService,private snack:MatSnackBar) { 
    ...
    this.minDate = new Date(currentYear - 50, 0, 0);
    this.maxDate = new Date(currentYear - 20, 0, 0);
  ...
  formSubmit(){
    ...
    || this.user.apellidos == '' || this.user.apellidos == null){
      this.snack.open('Los nombres y apellidos son requeridos !!','Aceptar',{
        duration : 3000,
        verticalPosition : 'top',
        horizontalPosition : 'right'
      });
    ...
    this.userService.guardarFisioterapeuta(this.user).subscribe(
      (data) => {
        console.log(data);
        location.reload()
        Swal.fire('Fisioterapeuta guardado','Fisioterapeuta registrado con exito en el sistema','success');
        ...
        if(error.status === 500){
          Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Ya existe un fisioterapeuta con ese Nombre de Usuario!!!',
          })
        }
