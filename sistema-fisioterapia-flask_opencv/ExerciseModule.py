...
class SimulateTargetExercices:
    ...
    def squats3(self, frame):
        ...
        if len(landmark_list) != 0:
            ...
            cv2.rectangle(img, (300, 100), (350, 400), color, 3)
            cv2.rectangle(img, (300, int(bar)), (350, 400), color, cv2.FILLED)
            cv2.putText(
                img, f"{int(per)} %", (300, 75), cv2.FONT_HERSHEY_PLAIN, 4, color, 4
            )
            cv2.circle(
    ...
    def squats1(self):
        print('holi')
        #ingresando el video del ejercicio
        cap = cv2.VideoCapture('C:/Users/asus/Desktop/Videos_Ejercicios/videos_grals/TrenInferior/Fortalecimiento/sentadilla.mp4')
        detector = pm.posture_detector()
        count = 0
        per = 0
        bar = 0
...
class StartWorkoutSession:
    ...
    @staticmethod
    def completion_screen(congrats_img):
    ...
    @staticmethod
    def calculate_calories(time_elapsed, weight, gender):
        total_calories = 0
    ...
    def complete_path(self):
        print('llegue aqui')
        warm_ups = SimulateWarmup(self.difficulty_level)
        target_exercises = SimulateTargetExercices(self.difficulty_level)
